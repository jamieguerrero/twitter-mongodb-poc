...
ptweets = harvest_user_timeline(
    twitter_api, screen_name=SCREEN_NAME, max_results=TWEETS_TO_ANALYZE
)
...
print('Most Positive Tweet: {0:6.3f} : "{1}"'.format(scores[most_positive], tweets[most_positive]['text']))
print('Most Negative Tweet: {0:6.3f} : "{1}"'.format(scores[most_negative], tweets[most_negative]['text']))
...
csv_file = df.to_csv(f"data/{SCREEN_NAME}.csv", index=None)
